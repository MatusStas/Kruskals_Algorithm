...
void fill_matrix(int *matrix_of_adjacency[], int *size, int *sorted_arr) {
	FILE *fr;
...
void show(int *new_matrix_of_adjacency[], int *size, int *cost) {
	for(int i = 0; i < *size; i++) {
		for(int j = 0; j < *size; j++) {
			printf("%d ", new_matrix_of_adjacency[i][j]);
			*cost+= new_matrix_of_adjacency[i][j];
		}
		putchar('\n');
	}
	printf("\nCOST: %d\n\n", *cost/2);
...
void sort(int *sorted_arr, int *size) {
	int temp;
	int sz = (*size)*(*size);
	for(int i = 0; i < sz; i++) {
		for(int j = 0; j < sz; j++) {
	...
	int not_zero = 0;
	while(sorted_arr[not_zero++] == 0) {;}
	for(int i = 0; i < sz; i++)
		sorted_arr[i] = sorted_arr[i + not_zero - 1];
	//sorted_arr = realloc(sorted_arr, (sz-not_zero-1)*sizeof(int));
}


int over(int *visited_vetrices, int *size) {
	for(int i = 0; i < *size; i++)
		if(visited_vetrices[i] == 0)
			return 1;
	return 0;
}


int bfs(int *matrix_of_adjacency[],int *size, int *start, int *end) {
	
	int sz = (*size)*(*size);
	int queue[sz];
	for(int i = 0; i < sz; i++)
		queue[i] = -1;
	int visited_vetrices[*size];
	for(int i = 0; i < *size; i++)
	...
	//printf("%d %d\n", *start,*end);
	while(visited_vetrices[*end] == 0) {
		if(queue[step] == -1)
			return 1;
		for(int i = 0; i < *size; i++) {

			if(matrix_of_adjacency[queue[step]][i] != 0 && visited_vetrices[i] == 0) {
				queue[add++] = i;
			}
			visited_vetrices[queue[step]] = 1;	
			if(visited_vetrices[*end]) {
				goto done;
			}
		}
		step++;
...
int main() {
	...
	int *sorted_arr = (int *) malloc(size*size*sizeof(int));

	int *matrix_of_adjacency[size];
	fill_matrix(matrix_of_adjacency, &size, sorted_arr);
	sort(sorted_arr, &size);

	int *new_matrix_of_adjacency[size];
	for(int i =0; i < size; i++) {
		new_matrix_of_adjacency[i] = (int *) malloc(size*sizeof(int));
		for(int j = 0; j < size; j++)
			new_matrix_of_adjacency[i][j] = 0;
	}
	...
	show(matrix_of_adjacency,&size,&cost);
	cost = 0;
	while(number_of_edges != size-1) {
		for(int i = 1; i < size; i++) {
			for(int j = 0; j < i; j++) {
						
				if(matrix_of_adjacency[i][j] == sorted_arr[step_sorted]) {		
					if(bfs(new_matrix_of_adjacency, &size, &i, &j)) {
						
						number_of_edges++;
						new_matrix_of_adjacency[i][j] = sorted_arr[step_sorted];
						new_matrix_of_adjacency[j][i] = sorted_arr[step_sorted];
						step_sorted += 2;
						//show(new_matrix_of_adjacency, &size, &cost);
						if(number_of_edges == size-1)
							goto done;
					}
					else{step_sorted+=2;}
				}
